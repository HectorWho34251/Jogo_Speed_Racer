class Game {
  ...
  getState() {
    //Lendo o estado de jogo do banco de dados (bd)
  ...
  start() {
    ...
    //adicionando o sprite do combustível
    this.addSprites(fuels, 4, fuel_img, 0.02);

    //adicionando o sprite de moeda
    this.addSprites(powerCoins, 18, powerCoin_img, 0.09);

    //adicionando o sprite dos obstáculos
  ...
  showLeaderboard() {
    ...
    //condição para identificar se o jogador 1 chegou em primeiro lugar
    if(players[0].rank == 0 && players[1].rank == 0 || players[0].rank == 1) {
      leader1 = players[0].rank + "&emsp;" + players[0].name + "&emsp;" + players[0].score;
      leader2 = players[1].rank + "&emsp;" + players[1].name + "&emsp;" + players[1].score;
    }
    
    //condição para identificar se o jogador 2 chegou em primeiro lugar
    if(players[1].rank == 1) {
  ...
  showFuelBar() {
    push();
    image(fuel_img, width / 2 - 130, height - player.positionY - 270, 20, 20);
    fill("white");
  ...
  handlePlayerControls() {
    if(!this.blast) {
      //manipulando os eventos do teclado
  ...
  handleFuel(index) {
    //coletando o combustível
    cars[index - 1].overlap(fuels, function(collector, collected){
      player.fuel = 185;
      //o sprite é coletado no grupo de combustíveis
      collected.remove();
    });
  ...
  handlePowerCoins(index) {
    ...
    cars[index - 1].overlap(powerCoins, function(collector, collected){
      player.score += 10;
      player.update();
      //o sprite é coletado no grupo de moedas
      collected.remove();
    });
  }
  ...
  showRank() {
    swal({
      title: `Você está em${"\n"}${player.rank}º lugar!`,
      text: "Parabéns, você chegou até a linha de chegada!",
      imageUrl:
  ...
  gameOver() {
    swal({
      title: `Que pena! A gasolina acabou`,
      text: "Na próxima vez, procure mais as gasolinas na pista!",
      imageUrl:
